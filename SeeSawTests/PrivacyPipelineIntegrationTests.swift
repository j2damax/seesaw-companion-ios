...
//
// The outer @Suite(.serialized) ensures all inner suites run serially with respect
// to each other, preventing concurrent VNClassifyImageRequest calls from saturating
...
//
// Outer .serialized guarantees that InvariantTests, PayloadSafetyTests, MetricsTests,
// ErrorTests, and ResultStructTests never run concurrently with each other.
